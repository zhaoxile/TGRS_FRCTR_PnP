...
function [clean_image,S,G, errList] = LRTDTV_G(Noi, Omsi,tau,lambda,beta,rank)
tic
sizeD           = size(Noi);
normD           = norm(Noi(:)); 
n               = prod(sizeD);
...
%     Z          = reshape(z,sizeD);
    ...
    mu         =  min(mu * 1.5,1e5); 
    %% compute the error,output SSIM and PSNR values of each step
    errList    = norm(X(:)-preX(:)) / normD; 
%     [out_value.PSNR(iter),out_value.SSIM(iter),out_value.ERGAS(iter)]=msqia(Omsi,X);
    psnr = PSNR3D(Omsi * 255, X * 255);
     if iter==1 || mod(iter, 10) == 0
    fprintf('LRTDTV: iterations = %d  psnr=%f  difference=%f\n', iter, psnr,errList);
     end
    if errList < epsilon
        break;  
    end 
